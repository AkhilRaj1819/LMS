...
const Module1_3: React.FC = () => {
  const quizQuestions = [
    {
      question: "Which part of an inductive proof is responsible for proving that the property holds for the smallest possible value?",
      options: ["Inductive Hypothesis", "Inductive Step", "Base Case", "Conclusion"],
      correctAnswer: 2,
      explanation: "The Base Case verifies the property for the initial value (like n=0 or n=1), anchoring the inductive chain."
    },
    {
      question: "What do you assume to be true in the 'Inductive Hypothesis' step of a standard induction proof?",
      options: [
        "The property holds for all n",
        "The property holds for some arbitrary value k",
        "The property holds for k+1",
        "The base case is false"
      ],
      correctAnswer: 1,
      explanation: "In the inductive hypothesis, we assume P(k) is true for some fixed k to see if that implies P(k+1) must also be true."
    },
    {
      question: "In Strong Induction, how does the Inductive Hypothesis differ from weak (standard) induction?",
      options: [
        "You assume the base case is true twice",
        "You assume the property is true for only k and k-1",
        "You assume the property is true for all values from the base case up to k",
        "There is no difference"
      ],
      correctAnswer: 2,
      explanation: "Strong Induction assumes P(i) is true for all i ≤ k, which provides more information to prove P(k+1) than just assuming P(k)."
    },
    {
      question: "Structural Induction is primarily used for proving properties of:",
      options: ["Summation formulas only", "Continuous functions", "Recursively defined structures like trees and lists", "Prime numbers only"],
      correctAnswer: 2,
      explanation: "Structural induction extends the idea of induction to structures that are defined in terms of smaller versions of themselves, such as trees, lists, or expressions."
    },
    {
      question: "If an inductive proof has a valid Inductive Step (P(k) → P(k+1)) but a missing or false Base Case, the proof is:",
      options: ["Still valid", "Invalid", "Only half-valid", "Valid for large n"],
      correctAnswer: 1,
      explanation: "A proof by induction requires BOTH a valid base case and a valid inductive step. Without the base case, the chain is never started."
    }
  ];
...
CONCLUSION:
  ...
  P(n) is true for all n ≥ n₀. ∎ (QED)`}</code>
      ...
      <section className="content-section mt-12 bg-slate-50 p-8 rounded-[3rem] border-2 border-slate-100">
        <h2 className="text-3xl font-black mb-8 text-center uppercase tracking-tighter italic">Module 1.3 Mastery Quiz</h2>
