...
print("Non-greedy:", non_greedy)`}</pre>
      ...
      <section className="content-section">
        <h3>12. MCQ Practice</h3>
        <p className="mt-4">Test your knowledge of regular expressions with these multiple-choice questions.</p>
        
        <Quiz 
          title="MCQ Practice: Regular Expressions"
          subject="Theory of Computation"
          unitId={1}
          moduleId={7}
          questions={[
            {
              question: "What is the language denoted by the regular expression (a+b)*?",
              options: [
                "The set of all strings ending in a or b",
                "The set of all strings over {a, b} of even length",
                "The set of all strings over {a, b} including the empty string",
                "The set of all strings starting with a and ending with b"
              ],
              correctAnswer: 2,
              explanation: "(a+b)* represents the Kleene closure of the set {a, b}, which contains every possible string that can be formed using zero or more characters from that set, including the empty string ε."
            },
            {
              question: "Which of the following describes the regular expression a*b*?",
              options: [
                "Strings consisting of any number of a's followed by any number of b's",
                "Strings that alternate between a and b",
                "Strings containing exactly one a and one b",
                "All strings over {a, b}"
              ],
              correctAnswer: 0,
              explanation: "a* matches zero or more a's, and b* matches zero or more b's. Concatenating them (a*b*) means all the a's must come before all the b's."
            },
            {
              question: "According to Arden's Lemma, if L = PL + Q and ε is not in P, what is the unique solution for L?",
              options: [
                "L = Q*P",
                "L = P*Q",
                "L = (P+Q)*",
              ...
              correctAnswer: 1,
              explanation: "Arden's Lemma states that L = P*Q is the unique solution to the equation L = PL + Q, provided ε is not in P."
            },
            {
              question: "What does the regular expression (ab)* differ from a*b*?",
              options: [
                "(ab)* only matches strings of even length, while a*b* matches strings of any length",
                "(ab)* matches alternating a's and b's, while a*b* matches a sequence of a's followed by a sequence of b's",
                "There is no difference; they accept the same language",
                "(ab)* matches strings starting with b, while a*b* does not"
              ],
              correctAnswer: 1,
              explanation: "(ab)* requires 'a' to be immediately followed by 'b' in pairs (e.g., ab, abab), while a*b* allows any number of a's to be followed by any number of b's (e.g., aaabb)."
            },
            {
              ...
              correctAnswer: 2,
              explanation: "Among the standard operators (+, ·, *), the Kleene Star (*) has the highest precedence, followed by concatenation, and then union. Parentheses are used to override this default precedence."
            },
            {
              question: "What is the minimum length of a string matched by the regex (a+b)c*d?",
              options: [
                "1",
                "2",
                "3",
              ...
              correctAnswer: 1,
              explanation: "The minimum length is achieved by picking one from (a+b), zero from c*, and the mandatory d. This gives 'ad' or 'bd', both of which have length 2."
            },
            {
              question: "In Thompson's Construction, how is R* handled?",
              options: [
                "By merging the start and accept states",
                "By adding a new start and accept state with ε-transitions for looping and skipping",
                "By reversing all transitions",
                "By duplicating the NFA for R"
              ],
              correctAnswer: 1,
              explanation: "Thompson's Construction for R* adds a new start state and a new accept state, connected via ε-transitions to allow skipping the NFA (for zero repetitions) or looping back (for multiple repetitions)."
            }
          ]}
        />
      </section>
