...
const Module1_6: React.FC = () => {
  const quizQuestions = [
    {
      question: "The Knuth-Morris-Pratt (KMP) algorithm achieves a matching time complexity of:",
      options: ["O(nm)", "O(n + m)", "O(n log m)", "O(n²)"],
      correctAnswer: 1,
      explanation: "KMP preprocessing takes O(m) and matching takes O(n), giving a total linear time complexity of O(n + m)."
    },
    {
      question: "What does the prefix function π[i] (LPS array) store?",
      options: [
        "The number of occurrences of the pattern",
        "The length of the longest proper prefix of P[0..i] which is also a suffix",
        "The next state in the DFA",
        "The index of the first mismatch"
      ],
      correctAnswer: 1,
      explanation: "The π array (or LPS - Longest Prefix which is also Suffix) stores lengths used to skip unnecessary comparisons during mismatches."
    },
    {
      question: "Which algorithm is most efficient for searching for multiple patterns simultaneously in a single text scan?",
      options: ["KMP", "Boyer-Moore", "Aho-Corasick", "Naive Matching"],
      correctAnswer: 2,
      explanation: "Aho-Corasick builds a trie-based automaton with failure links to search for multiple patterns in O(n + z) time (where z is number of matches)."
    },
    {
      question: "Why is automata-based text search more efficient than the naive approach?",
      options: [
        "It uses less memory",
        "It eliminates backtracking in the text during scanning",
        "It only works for small alphabets",
        "It uses hardware acceleration"
      ],
      correctAnswer: 1,
      explanation: "By preprocessing the pattern into an automaton, we can determine the next state without ever moving backward in the input text."
    },
    {
      question: "For very large alphabets and long patterns, which algorithm often performs best in practice by skipping characters?",
      options: ["KMP", "Boyer-Moore", "Rabin-Karp", "Aho-Corasick"],
      correctAnswer: 1,
      explanation: "Boyer-Moore matches from right-to-left and uses 'bad character' and 'good suffix' rules to skip potentially many characters in the text."
    }
  ];
  return (
 ...
 qA                                      qA'
  ...
  [NFA for R]  ─────────►  [NFA for S]  ─────► qA'
      ...
      <section className="content-section mt-12 bg-slate-50 p-8 rounded-[3rem] border-2 border-slate-100">
        <h2 className="text-3xl font-black mb-8 text-center uppercase tracking-tighter italic">Module 1.6 Mastery Quiz</h2>
        <div className="bg-white p-6 rounded-xl border-2 border-slate-100 shadow-sm transition-all hover:shadow-md">
          <Quiz
            title="Text Search & Automata Quiz"
      ...
      <div className="mt-16 py-8 border-t text-center opacity-30">
        <p className="text-[10px] font-black uppercase tracking-[1em]">Unit 1.6 | Module Complete</p>
      </div>
